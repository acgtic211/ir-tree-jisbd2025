...
import org.ual.querytype.range.RangeQuery;
...
public class QueryLogic {
    ...
    public QueryLogic(IndexLogic indexLogic, StatisticsLogic statisticsLogic, String resultsDirectoryPath, boolean writeQueriesToDisk) {
    ...
    public QueryLogic(IndexLogic indexLogic, StatisticsLogic statisticsLogic, String resultsDirectoryPath, boolean writeQueriesToDisk, int[] groupSizes,
    ...
    public void processAggregateQuery(AggregateQueryType[] aggregateQueryTypes, ArrayList<QueryType> queryTypes, String aggregator) {
        // Query evaluation
...
//    }
    ...
    private QueryStatsData evaluateAggregateQuery(AggregateQueryType aggregateQueryType, QueryType queryType, int groupSize, int mPercentage, int numberOfKeywords, double querySpaceAreaPercentage,
                                                   double keywordSpacePercentage, int topk, double alphaDistribution, String aggregator) {
        ...
        if (aggregateQueryType == AggregateQueryType.GNNK || aggregateQueryType == AggregateQueryType.GNNK_BL) {
            ...
            for (SGNNKQuery q : sgnnkQueries) {
                if (aggregateQueryType == AggregateQueryType.SGNNK) {
                    ...
                    Map<Integer, List<SGNNKQuery.Result>> results = tree.sgnnkExtended(IndexLogic.invertedFile, q, topk);
                    ...
                    while (q.subGroupSize <= q.groupSize) {
                        ...
                        List<SGNNKQuery.Result> results = tree.sgnnk(IndexLogic.invertedFile, q, topk);
                        if(writeQueriesToDisk)
                            resultWriter.writeSGNNKResult(results);
