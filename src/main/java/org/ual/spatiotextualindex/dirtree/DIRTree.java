...
public class DIRTree extends RTreeEnhanced implements ISpatioTextualIndex {
...
//					System.out.println(prunningBound);
        ...
        while (!queue.isEmpty() /*&& results.size() < 10*/) {
            ...
            if(rtreeEntry.isLeafEntry) {
                ...
                for (int child = 0; child < node.children; child++) {
                    ...
                    //log.debug("Node Type {}, Node level: {}", node.type, node.level);
                    if (node.level == 0) {
            ...
            if(rtreeEntry.isLeafEntry) {
                ...
                for (int child = 0; child < node.children; child++) {
                    ...
                    if(var == null)
                        ...
                        int hit = var;
                        if(hit < query.query.keywords.size()) //TODO CHANGE for !=
                            continue;
                    }
    ...
    public List<SKNNQuery.Result> topkKnnQuery(InvertedFile invertedFile, SKNNQuery query, int topk) {
        ...
        while (!queue.isEmpty()) {
            ...
            if (rtreeEntry.isLeafEntry) {
                if (count >= topk || first.cost > knearest) // TODO CHANGED AND TO OR
                    break;
                ...
                invertedFile.load(node.identifier);
                if (numOfClusters != 0)
